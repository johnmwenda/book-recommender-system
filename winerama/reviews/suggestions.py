...
def update_clusters():
    ...
    print((num_reviews % update_step) == 0)
    return
    if num_reviews % update_step is False:
        ...
        for i in range(num_users):  # each user corresponds to a row, in the order of all_user_names nopep8
            user_reviews = Review.objects.filter(user_name=all_user_names[i])
            for user_review in user_reviews:
                ratings_m[i, user_review.wine.id] = user_review.ratings_m
